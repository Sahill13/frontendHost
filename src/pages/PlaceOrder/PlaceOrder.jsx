...
const PlaceOrder = () => {
  ...
  const { getTotalCartAmount, token, food_list, cartItems, url, setOrderId,setWaitingForApproval,startPolling } = useContext(StoreContext);
  ...
  const redeemAmount = location.state?.redeemAmount || 0;  

  // ✅ Compute totalAmount safely
  useEffect(() => {
    ...
    for (const itemId in cartItems) {
      ...
      if (item && item.price) {
        calculatedTotal += item.price * cartItems[itemId];
      }
    }
  ...
  const onChangeHandler = (event) => {
    ...
    // ✅ Restrict special characters and numbers in names
  ...
  if (data.phone.length !== 10 || !/^[6-9]\d{9}$/.test(data.phone)) {
    ...
    let orderItems = food_list
      ...
      const discountedAmount = totalAmount + (data.orderType === 'Takeaway' ? 0 : 2) - redeemAmount;

    const orderData = {
      userId: localStorage.getItem("userId"),  // ✅ Include userId
      address: data,
      items: orderItems,
      amount: discountedAmount, // ✅ Send corrected final amount
      status: 'pending',
      cafeteriaId,
    ...
    console.log("📌 Sending Order Data:", orderData); // 🔍 Debugging order payload
    ...
    try {
      ...
      if (success) {
        localStorage.setItem("pendingOrderId", orderId); // ✅ Save orderId to local storage
        setOrderId(orderId);
        setWaitingForApproval(true);
        localStorage.removeItem("redeemAmount");
        sessionStorage.removeItem("redeemAmount"); // ✅ Clear sessionStorage first
        toast.info('Your order is awaiting admin approval');
        startPolling(orderId); // 🔥 This triggers Razorpay without needing to reload
        navigate('/myorders');
      } else {
      ...
      <div className="place-order-right">
        <div className="cart-total">
          ...
          <div>
            ...
            <hr />
            <div className="cart-total-details"><p>Delivery Fee</p><p>₹{(data.orderType === 'Takeaway' || totalAmount === 0) ? 0 : 2}</p></div>
            <hr />
            <div className="cart-total-details"><p>SuperCoin Discount</p><p>-₹{redeemAmount}</p></div>
            <hr />
            <div className="cart-total-details"><b>Total</b><b>₹{totalAmount + 2 - redeemAmount}</b></div>
          </div>
